...
import type { SessionSummary } from '@CloudCraft/shared';
      ...
      <div className="max-w-4xl mx-auto px-8 py-8">
        ...
        {!authLoading && !user && (
          <div className="text-center py-16">
            ...
            <Link
              to="/"
              className="inline-flex items-center mt-4 px-4 py-2 text-sm font-medium text-white bg-orange-600 hover:bg-orange-700 rounded-lg transition-colors"
            >
              Back to Home
            </Link>
          </div>
        ...
        {sessions.length > 0 && (
          <div className="space-y-2">
            {sessions.map((s) => (
              <div
                key={s.id}
                className="flex items-center gap-4 p-4 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-orange-500/50 dark:hover:border-orange-500/30 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
              >
                {/* Provider badge */}
                <span
